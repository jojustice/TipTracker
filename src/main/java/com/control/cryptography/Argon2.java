package com.control.cryptography;

/*
 * Adapted from Argon2 class here:
 *      https://github.com/SoftwareVerde/java-cryptography/
 *
 * MIT License, full text here:
 *      https://github.com/SoftwareVerde/java-cryptography/blob/master/LICENSE
 */
...
public class Argon2 {
    public static final int DEFAULT_ARGON2_TYPE = Argon2Parameters.ARGON2_id;
    public static final int DEFAULT_ARGON2_VERSION = Argon2Parameters.ARGON2_VERSION_13;
    public static final int DEFAULT_ITERATION_COUNT = 12;
    public static final int DEFAULT_MEMORY_FOOTPRINT_POWER_OF_2 = 16; // 2**16 kB = 64MB
    public static final int DEFAULT_THREAD_COUNT = 2;
    public static final int DEFAULT_SALT_LENGTH_BYTES = 16;
    public static final int DEFAULT_OUTPUT_BYTE_COUNT = 32;
    ...
    public byte[] generate(final String password) {
    ...
    private String _getParameterString() {
        return "Argon2$" +
                ...
                _iterations +
                "$" +
                _allowedMemoryPowerOf2 +
                "$" +
                _threadCount +
                "$" +
                Hex.toHexString(_salt) +
                "$" +
                _outputByteCount;
    }
    ...
    public String generateParameterizedHash(final String password) {
        return generateParameterizedHash(password.toCharArray());
    }

    private String _toParameterizedHash(final byte[] result) {
        return _getParameterString() + "$" + Hex.toHexString(result);
    }
}